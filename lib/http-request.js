...
var nock;
if (process.env.NODE_ENV === 'obsidian-http-request-test') {
    nock = require('nock'); // 仅在测试环境中导入
}
...
function _request(requestUrl, options) {
    ...
    requestUrl = url.resolve(global.location?.href || '', requestUrl);
...
function _requestProxy(requestUrl, options) {
    ...
    return new Promise((resolve, reject) => {
        if (process.env.NODE_ENV === 'obsidian-http-request-test') {
            nock('http://localhost')
                .post(httpRequest.proxyPath)
                .reply(200, 'Proxied test response');
        }
        var req = http.request(httpOptions, resolve)
            .on("error", reject);
...
httpRequest = {
     ...
     * @return {Promise}
     */
    getRaw: function(url, options, callback) {
     ...
     * @return {Promise}
     */
    getBlob: function(url, options, callback) {
        const promise = _request(url, options)
            .then(_checkHeaders)
            .then(_readBody)
            .then(_bodyToBlob)
     ...
     * @return {Promise}
     */
    getText: function(url, options, callback) {
        const promise = _request(url, options)
            .then(_checkHeaders)
            .then(_readBody)
            .then(_bodyToString)
    ...
    getRawProxy: function(url, options, callback) {
        const promise = _requestProxy(url, options)
            .then(_checkHeaders)
            .then(_readBody)
            .then(_returnBody)
            .catch(function (error) {
